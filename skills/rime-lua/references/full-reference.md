# Rime Lua Full Reference

## 資源：librime.lua

[librime.lua](../assets/librime.lua) 是供 [lua-language-server](https://github.com/LuaLS/lua-language-server) 使用的類型標注檔（`---@meta`），包含所有 Rime Lua 物件的類型定義，可提供完整的補全與型別檢查。
